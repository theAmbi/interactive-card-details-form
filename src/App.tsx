...
function App() {
  ...
  const handleSubmit = (e: FormEvent) => { // Changed from SubmitEvent to FormEvent
    e.preventDefault();
    ...
    if (!newErrors.fullName && !newErrors.cardNumber && !newErrors.expiryMonth && !newErrors.expiryYear && !newErrors.cvc) {
  ...
  return (
    <section className='w-full h-screen'>
      <form action={''} onSubmit={handleSubmit} className='lg:hidden'>
        ...
        {!formSubmitted && <div className="details mt-20 flex flex-col gap-10 px-10 ">
          ...
          <div className="cardnumber flex flex-col gap-3">
            ...
            <input type="text" placeholder='e.g 1234 5678 9123 000' className={` rounded-md border w-full px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${cardNumber ? 'border-grey-200' : 'border-red-500'}`} value={cardNumber} onChange={handleCardNumberChange} />
          </div>
          ...
          <div className='flex items-center justify-between gap-5'>
            <div className="expiry flex flex-col gap-3 w-1/2 ">
              ...
              <div className='flex gap-3'>
                <input type="number" placeholder='MM' maxLength={2} className={`rounded-md border w-24 px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${expiryMonth ? 'border-grey-200' : 'border-red-500'}`} value={expiryMonth} onChange={handleMonthChange} />
                <input type="number" placeholder='YY' maxLength={2} className={`rounded-md border w-24 px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${expiryYear ? 'border-grey-200' : 'border-red-500'}`} value={expiryYear} onChange={handleYearChange} />
              </div>
            ...
            <div className="cvc flex flex-col gap-3 w-1/2">
              ...
              <input type="number" placeholder='e.g 123' maxLength={3} className={`rounded-md border w-full px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${cvc ? 'border-grey-200' : 'border-red-500'}`} value={cvc} onChange={handleCVCChange} />
            </div>
          </div>
      ...
      <div className='w-full h-screen relative flex justify-center items-center'>
        ...
        <div className='w-[400px] h-full absolute left-0 top-0 bg-cover z-0' style={{ backgroundImage: `url(${desktopBG})` }} ></div>

        <div className=" content absolute inset-0 flex items-center justify-center z-10 ">
          {/* cards  */}
          <div className=" flex items-center justify-center gap-8 bg-transparent">
            <div className="relative bg-transparent bg-none">

              {/* card front  */}
              <div className='relative'>
                <div className='w-fit relative bg-transparent'>
                  <img src={cardLogo} className='absolute top-10 px-6' />
                  <img src={cardFront} alt="card-front" className='bg-transparent bg-none shadow-2xl' />
                  <p className='absolute w-full px-6 bottom-[72px] text-left text-white text-3xl font-medium tracking-widest'>{cardNumber}</p>
                  <div className='w-full text-white text-sm font-semibold uppercase tracking-wide flex justify-between items-center px-6 absolute bottom-7'>
                    <p>{fullName}</p>
                    <p>{expiryMonth}/{expiryYear}</p>
              ...
              {/* Card back */}
              <div className='w-fit relative ml-24 mt-14'>
                <img src={cardBack} alt="card-back" className='bg-transparent bg-none shadow-2xl' />
                <p className='absolute right-16 top-[108px] text-right text-sm text-white font-medium tracking-widest'>{cvc}</p>
              </div>
            </div>
            ...
            {!formSubmitted && <form className="details mt-20 flex flex-col gap-10 px-10 max-w-lg">
              ...
              <div className="cardnumber flex flex-col gap-3">
                <label htmlFor="card-number" className='text-sm uppercase font-medium tracking-widest text-purple-950'>Card number</label>
                <input type="text" placeholder='e.g 1234 5678 9123 000' className={` rounded-md border w-full px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${errors.cardNumber ? 'border-red-500' : 'border-grey-200'}`} value={cardNumber} onChange={handleCardNumberChange} />
              </div>
              ...
              <div className='flex items-center justify-between gap-5'>
                <div className="expiry flex flex-col gap-3 w-1/2 ">
                  <label htmlFor="exp-date" className='text-sm uppercase font-medium tracking-widest text-purple-950'>Exp. date (mm / yy)</label>
                  <div className='flex gap-3'>
                    <input type="number" placeholder='MM' maxLength={2} className={`rounded-md border w-24 px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${errors.expiryMonth ? 'border-red-500' : 'border-grey-200'}`} value={expiryMonth} onChange={handleMonthChange} />
                    <input type="number" placeholder='YY' maxLength={2} className={`rounded-md border w-24 px-3 py-4 text-lg font-medium placeholder:text-md placeholder:font-medium placeholder:text-grey-200 outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-300 ease-in-out ${errors.expiryYear ? 'border-red-500' : 'border-grey-200'}`} value={expiryYear} onChange={handleYearChange} />
                  </div>
      ...
      {
        formSubmitted && <div className='flex flex-col items-center justify-center gap-10 py-14 mt-20 px-10 lg:hidden' id='success'>
